...
function start_geth() {
    ...
    rm -rf "${BASE_DIR}/etc/execution/node${NUM}"
    ...
    geth init --state.scheme "hash" --datadir "${BASE_DIR}/etc/data/execution/node${NUM}" "${BASE_DIR}/etc/config/genesis.json" > "$LOG_FILE" 2>&1
    geth --datadir "${BASE_DIR}/etc/data/execution/node${NUM}" \
         --state.scheme "hash" \
         --networkid 212121 \
         --authrpc.vhosts "*" \
