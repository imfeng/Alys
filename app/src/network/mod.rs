...
mod test {
    ...
    async fn test_network_handler() {
        ...
        let client = spawn_network_handler(55444, Some("/ip4/192.168.4.70/tcp/55444".to_string())).await.unwrap();
        
        let mut events = client.subscribe_events().await.unwrap();
        
        let received_event = events.recv().await.unwrap();
        // let mut rpc_events = client.subscribe_rpc_events().await.unwrap();
        let mut peers = client.subscribe_peers().await.unwrap();
        
        let received_peers = peers.recv().await.unwrap();
